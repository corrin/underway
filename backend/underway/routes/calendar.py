"""Calendar API routes — events, create, delete, set-primary."""
...
async def list_events(request: Request) -> dict[str, object]:
    """GET /api/calendar/events?start=...&end=... — list events from primary calendar."""
    ...
    account = await ExternalAccount.get_primary_account(session, user.id, "calendar")
    if not account:
...
async def set_primary(request: Request) -> dict[str, str]:
    ...
    return {"status": "ok", "message": f"{account.external_email} set as primary calendar."}
